...
- **LIKE 模式中的用户输入必须转义 %/_ 并配 ESCAPE 子句** *(2026-08-29 验证修复)*：搜索曾把用户关键词直接拼成 `"%" + query + "%"` 传给 `NAME LIKE ?`——`%`/`_` 在 SQLite LIKE 模式中是通配符且无 ESCAPE，输入 `100%` 实际按 `100` 匹配、单独 `%` 命中全部节点。修复：`OrgFileRepository.likePattern(keyword)` 静态方法统一做 trim + 转义（`%`→`\%`、`_`→`\_`、`\`→`\\`），`search()` 的 selection 加 `ESCAPE '\'`；调用方 `SearchActivity.doSearch` 不再自行拼模式。通用规则：任何把外部可控值放进 LIKE 模式的地方都必须同样转义+ESCAPE。同日按此规则修复同类三处：`OrgNodeRepository.updateAllNodes`（手写 `:ID:` 值含 `%` 时 LIKE 匹配过宽 → 错误覆写其他节点，2026-08-22 事故同族）、`findOriginalNode`（拼接 SQL，`'` 断句抛 SQLiteException；改参数化）、`OrgQueryBuilder.getLikeSelection`（agenda 过滤值直接拼进 SQL，注入+通配；改参数化+ESCAPE）。剩余低风险点暂留：`CalendarWrapper` 的 filename LIKE（参数化但未转义，文件名含 `%`/`_` 罕见）、`OrgProvider` SEARCH 分支 `name LIKE %?%` 非法 SQL（疑似死代码）。测试坑：`stripProperties()` 提取 `:ID:` 值要求行尾有换行——构造测试 payload 必须写 `":ID: xxx\n"`，否则 getId() 返回 null 走 olp: 分支，测试伪通过。
...
- **开关型 preference 与其依赖项是独立配置，半配置状态必须被 Service 入口守卫** *(2026-09-05 修复日历同步崩溃)*：`calendarEnabled`（CheckBox）勾选后，`calendarName`（ListPreference，XML 默认值 `""`）可以永远没被选择——UI 上无任何提示（ListPreference summary 是静态文案，不显示当前值），且 `CalendarSyncService.onSharedPreferenceChanged` 监听所有 `calendar` 前缀 key，勾选开关的**瞬间**就在主线程跑全量 push；org 同步完成后 `SyncService` 也只检查开关就发起 push。空名使 `getCalendarID("")` 必返回 -1，`insertEntry` 抛 `IllegalArgumentException("Couldn't find selected calendar: ")`（冒号后空白=空名的特征），整条调用链无 catch，后台线程未捕获异常炸进程。修复：`CalendarWrapper.isCalendarSelected()`（name 非空且 id != -1）+ `CalendarSyncService` 两个 `syncFiles` 重载入口统一守卫（一个点覆盖 onStartCommand push 分支、onSharedPreferenceChanged、未来新调用方），跳过时 `Log.w` 留可观测信号。通用规则：(1) "启用开关"与"完成该功能全部配置"之间是用户可达的常态状态，Service/入口层必须按"配置不完整则跳过并留日志"处理，不能假设用户走完了设置流程；(2) throws 声明的 API（如 `insertEntry` 的 IllegalArgumentException）必须有调用方处理链，否则删掉声明或接入口守卫；(3) instrumentation 测试 shell-grant 权限后绝不 revoke（revoke 杀 instrumentation 进程），依赖 denied 态的测试类（Gui.SettingsCalendarPermissionTest）必须按字母序排在 grant 类之前——包名 Gui < Services 是隐式契约，注释已写明。
...
- **instrumentation 中 `pm revoke` 已授予的权限会杀死 app 进程（=instrumentation 自己）** *(2026-08-28 TDD 发现，MI PAD 4 / MIUI / API 27)*：MIUI PowerKeeper（部分 AOSP 版本同样）在运行时权限被撤销时 force-stop 该 app，而 instrumentation 恰好跑在 app 进程内——测试体内 `pm grant` 后若 @After 里 `pm revoke` 恢复现场，整个测试运行直接 "Process crashed"。规则：(1) 需要 grant 的用例必须排到最后（`@FixMethodOrder(MethodSorters.NAME_ASCENDING)` + 方法名编号）；(2) @After 绝不 revoke——gradle 的 connectedAndroidTest 每次跑完会卸载重装两个 APK，下次运行必然回到默认 denied 态，无需跨运行恢复。
- **非 exported 的 Activity 无法用 shell `am start` 启动**：`am start -n pkg/.NonExportedActivity` 以 shell uid（2000）执行会被拒（`Permission Denial: ... not exported from uid`），测试里启动应用内部页面应改用 `targetContext.startActivity(intent + FLAG_ACTIVITY_NEW_TASK)`（同进程同 uid 合法）。
- **MIUI 权限弹窗的 mCurrentFocus 焦点检测不可靠，用被测 activity 生命周期检测**：MIUI 的 LBE 权限对话框弹出时 `dumpsys window` 的 mCurrentFocus 未必移出 app 包名，按焦点轮询会漏检。可靠信号：权限对话框必然 pause 被覆盖的 activity——`ActivityScenario.launch()` 返回（首次 RESUMED）后，正向断言轮询 `scenario.getState() == Lifecycle.State.STARTED`（注意 Lifecycle.State 没有 PAUSED 粒度，被对话框覆盖降为 STARTED）；负向断言轮询期间恒为 RESUMED。

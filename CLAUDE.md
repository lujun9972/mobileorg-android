...
- **AlarmManager is unreliable for exact timing on modern Android**: (1) `setRepeating()` has been inexact since API 19 — alarms are batched and can fire minutes late. (2) `setWindow()` with a window < 10 minutes is forcibly elongated to 10 minutes on API 31+ (Android 12+), so `setWindow(RTC_WAKEUP, trigger, 60000)` fires anywhere in a 10-minute window. (3) `setExact()` and `setExactAndAllowWhileIdle()` require `SCHEDULE_EXACT_ALARM` permission on API 31+; without it, calls throw SecurityException or are silently degraded. (4) Doze and App Standby further delay all non-exact alarms. **Fix for foreground service timers (TimeclockService)**: Use `Handler.postDelayed()` instead of AlarmManager — the foreground service is always alive, so Handler timing is both simpler and perfectly reliable. **Fix for background reminders (ReminderScheduler)**: `SCHEDULE_EXACT_ALARM` is a **special** permission on API 31+ — manifest declaration alone is insufficient, the user must grant it in system settings; a fresh install defaults to denied. Centralize the strategy choice in `ReminderScheduler.chooseAlarmStrategy(apiLevel, canScheduleExactAlarms)`: API 31+ without the permission must fall back to `setWindow()` (NOT `setExactAndAllowWhileIdle()` — that still throws `SecurityException`); API 23-30 uses `setExactAndAllowWhileIdle()` without special permission; daily overview uses one-shot `setExact()` (not `setRepeating()`) rescheduled in `DailyOverviewReceiver`.

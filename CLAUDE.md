...
- **LIKE 模式中的用户输入必须转义 %/_ 并配 ESCAPE 子句** *(2026-08-29 验证修复)*：搜索曾把用户关键词直接拼成 `"%" + query + "%"` 传给 `NAME LIKE ?`——`%`/`_` 在 SQLite LIKE 模式中是通配符且无 ESCAPE，输入 `100%` 实际按 `100` 匹配、单独 `%` 命中全部节点。修复：`OrgFileRepository.likePattern(keyword)` 静态方法统一做 trim + 转义（`%`→`\%`、`_`→`\_`、`\`→`\\`），`search()` 的 selection 加 `ESCAPE '\'`；调用方 `SearchActivity.doSearch` 不再自行拼模式。通用规则：任何把外部可控值（用户输入、文件名、节点标题派生的 olp 路径）放进 LIKE 模式的地方都必须同样转义+ESCAPE——仓库中 `OrgNodeRepository.updateAllNodes/findOriginalNode`（nodeId 拼模式，2026-08-22 事故同族）与 `OrgQueryBuilder.getLikeSelection`（agenda 过滤值直接字符串拼接，含单引号还会破坏 SQL）尚未修复。

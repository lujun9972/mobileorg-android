...
public class OrgRenderer {
            ...
            switch (state) {
                ...
                case STATE_QUOTE:
                    if (trimmed.startsWith("#+END_QUOTE")) {
                        // End of quote block
                        String quoteContent = blockBuffer.toString().trim();
        ...
        for (String line : lines) {
            ...
            if (firstRow) {
                ...
                for (String cell : cells) {
                    result.append("<th>")
                          .append(applyInlineMarkup(cell.trim()))
                          .append("</th>");
                }
                result.append("</tr>\n</thead>\n<tbody>\n");
                firstRow = false;
                ...
                result.append("<tr>");
                for (String cell : cells) {
                    String cellContent = applyInlineMarkup(cell.trim());
    ...
    public String toHTML(OrgNode node, int levelOfRecursion) {
        String htmlBody = nodeToHTMLRecursive(node, levelOfRecursion);
    ...
    private String nodeToHTMLRecursive(OrgNode node, int level) {
        ...
        if (level == 0) {
            ...
            for (OrgNode child : node.getChildren(resolver)) {
                result.append(nodeToHTMLRecursive(child, 0));
            }
        } else if (level > 0) {

...
- **saveClock must preserve original clock-in time**: `TimeclockDialog.saveClock()` was calculating `startTime = endTime - duration`, which shifts the entire time window when the user edits the duration. The original clock-in time from `TimeclockService.getStartTime()` should be preserved, and `endTime` should be calculated as `startTime + duration`. This way editing the interval from 28min to 2min produces `[07:01]--[07:03]` instead of `[07:27]--[07:29]`.
- **Do not use TimePickerDialog for duration input**: `android.app.TimePickerDialog` on some devices (confirmed on API 34 with MaterialComponents theme) returns initial values in `onTimeSet()` callback params AND `view.getHour()/getMinute()` — user's selection is completely ignored. This makes it impossible to edit clock duration. Fix: replaced with `AlertDialog` + two `NumberPicker` widgets for hours/minutes in `DurationPickerFragment`. `NumberPicker.getValue()` reliably returns user-selected values.
